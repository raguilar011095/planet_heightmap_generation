...
export default definePass({
  ...
  run(world, p, ctx) {
    ...
    tick('relocate');
    // 5. Gaps: spreading (stretched continent, then ocean floor), rounding dilation, orphans.
    const gf = fillGaps({ world, p, next, filled, gapList, nb1, nb2, kind, CRUST_FIELDS });
    const { gaps, orphans, stretched } = gf;
    dilated += gf.dilated; mDilateIn += gf.mDilateIn; mDilateOut += gf.mDilateOut;
    tick('gapFill');
