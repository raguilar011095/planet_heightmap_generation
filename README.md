...
- **Ocean floor features** — mid-ocean ridges at divergent boundaries with half-space-cooling bathymetry (seafloor deepens with the square root of distance from the ridge, mean abyssal depth preserved), trenches at subduction zones with an axis-taper + outer-rise bulge profile, fracture zones at transform boundaries with a lateral texture offset that juxtaposes abyssal noise across the fault, back-arc basins behind subduction zones
- **Island arcs** — volcanic island chains at ocean-ocean convergent boundaries with ridged noise shaping
- **Hotspot volcanism** — dual-component mantle plume model (broad thermal swell + volcanic peak) with drift-trail island chains, domain-warped shape distortion, drift-direction elongation, summit calderas on active domes, radial rift-zone ridges, age-dependent volcanic texture, and per-hotspot variation in strength/decay/spacing; the number of hotspots is adjustable via the Hotspots slider
- **Terrain post-processing** — noise-based domain warping (FBM simplex noise with greedy mesh walk) to deform the elevation field for organic coastlines and mountain ridges, independently controllable bilateral smoothing to blend harsh BFS distance-field boundaries, glacial erosion that carves fjords, U-shaped valleys, and lake basins at high latitudes and altitudes via latitude-driven ice flow with drainage accumulation, priority-flood pit resolution with canyon carving (Barnes et al. algorithm that ensures every land cell drains to the ocean, carving dramatic canyons through mountain saddle points rather than filling basins), iterative implicit stream power hydraulic erosion (Braun-Willett style) — modulated by lithology-driven erodibility so cratons resist erosion and sedimentary basins erode faster — that carves self-reinforcing river valleys with capacity-limited sediment transport depositing excess material in flat receivers and ocean deltas (controlled by the Deposition slider), thermal erosion that softens ridges via talus-angle material transport, ridge sharpening that accentuates mountain ridgelines, always-on soil creep (Laplacian diffusion) that rounds off hillslopes, and flexural isostatic rebound (controlled by the Rebound slider) that flexes eroded terrain back upward over a ~200 km radius so old orogens keep broad relief instead of melting away
...
- **Rivers** — a drainage graph extracted from the final, fully-eroded terrain via steepest-descent flow routing to the ocean, with flow accumulation re-weighted by simulated precipitation once climate has run. Rendered as flow-ramped polylines (pale blue → deep blue) on both the globe and equirectangular map, with a Chaikin corner-cut smoothing pass so low-Detail rivers don't read as cell-center zigzags. On by default; toggle with the Rivers switch in Visual Options
...
- **Ocean surface currents** — wind-coupled model blending a rule-based geographic gyre simulation (driven by wind belts — trade winds, westerlies, polar easterlies — with a longitude-varying ITCZ equatorial countercurrent) 60/40 with the actual simulated seasonal wind field rotated by a 35° Ekman angle (`OCEAN_WIND_COUPLING = 0.6` in `js/climate-config.js`, chosen via the Earth-match gate). Continental shelves are classified as western or eastern boundaries via coast-normal BFS, producing subtropical gyres (CW in NH, CCW in SH) with western boundary intensification (Gulf Stream, Kuroshio effect) and weaker eastern boundary return flow. The wind-coupling lets currents deflect around embayments and reverse seasonally on monsoon coasts. Detects circumpolar channels for unobstructed eastward currents (Antarctic Circumpolar Current). Currents are colored by heat transport: red = warm poleward flow, blue = cold equatorward flow, black = zonal (neutral). Computed for both summer and winter seasons.
...
- **Map type switcher** — first-class Terrain / Satellite / Climate / Trewartha / Heightmap tabs with color legends for each view
...
Every generated planet produces a **planet code** (shown below the Build button) that encodes the random seed, all slider values, and any plate edits. An unedited planet is 25 characters; plate edits (applied via Rebuild) extend the code to include the toggled plates. Older codes (13, 14, 16, 17, 18, 21, or 22 characters) from previous versions are still supported — missing sliders (including Deposition, Rebound, and Hotspots, added in later versions) default to their current default values. To share a planet:
...
| Hotspots | 0 – 12 | 5 | Number of mantle hotspots — each spawns a volcanic island chain or continental volcanic field |
...
| Deposition | 0 – 1 | 0.5 | Where eroded sediment settles — low exports it to the sea for sharp canyons, high builds floodplains and river deltas |
| Rebound | 0 – 1 | 0.35 | Isostatic rebound — the crust flexes back up where erosion removes weight, letting old mountain ranges keep broad relief |
...
- **Map Type** — segmented Terrain / Satellite / Climate / Trewartha / Heightmap tabs for quick switching between the five most common visualizations. Each tab shows a color legend:
...
- **Rivers** — toggle switch to show or hide the drainage-graph river overlay on the globe and map. On by default.
...
- **Main views** (ungrouped at top) — Terrain, Satellite, Köppen Climate, Trewartha Climate, Land Heightmap
...
- **Atmosphere** — Pressure Summer/Winter (blue = low, red = high), Wind Speed Summer/Winter (with directional arrows on both globe and map), Cloud Cover Summer/Winter (display-only cloud fraction derived from precipitation)
...
- **View switcher** — a dropdown in the top-right lets you switch between Terrain, Satellite, Climate, Trewartha, and Heightmap views without opening the bottom sheet.
...
- **Half-space cooling bathymetry** — ridge-age seafloor depth follows a square-root-of-distance subsidence curve from the ridge axis, gated beyond the continental margin and centered to preserve mean abyssal depth
- **Trench band profile** — an axis-taper fade combined with a sinusoidal outer-rise bulge, replacing a flat elevation subtraction at subduction trenches
- **Transform-fault texture offset** — abyssal noise sampled from a tangentially shifted position on each side of a fracture-zone BFS, juxtaposing terrain texture across the fault
- **Lithology-driven erodibility** — a craton/basin hardness field modulates hydraulic erosion rate so cratons resist erosion and sedimentary basins erode faster
- **Capacity-limited sediment transport** — excess eroded material is deposited in flat receivers and ocean deltas rather than vanishing, controlled by the Deposition slider
- **Flexural isostatic rebound** — erosion-driven mass loss is diffused over a flexural radius and partially added back to elevation, controlled by the Rebound slider
- **Wind-coupled ocean currents** — rule-based gyre currents are blended 60/40 with the seasonal wind field rotated by a 35° Ekman angle (`OCEAN_WIND_COUPLING = 0.6` in `js/climate-config.js`); the coupling weight was chosen via the Earth-match gate (`tuning/climate/evaluate.mjs` objective 0.6778542 vs. 0.6778484 for the pre-coupling baseline)
- **Steepest-descent river drainage** — a drainage graph is extracted from the final terrain via steepest-descent flow routing to the ocean, with flow accumulated and re-weighted by simulated precipitation
...
js/
  ...
  trewartha.js          Trewartha climate classification — parallel scheme to Köppen using the same seasonal proxies
...
tuning/
  ...
  check-planet-code.mjs Round-trip + legacy-decode regression check for the planet-code format
```

The tuning suite has its own dependencies on top of the app itself: run `npm install` in the project root to pull in `pngjs`, `delaunator`, and `puppeteer` (headless-browser scoring and screenshot regression). `tuning/climate/evaluate.mjs` additionally needs a local copy of the Köppen-Geiger ground-truth grid — download it from `https://koeppen-geiger.vu-wien.ac.at/data/Koeppen-Geiger-ASCII.zip` and unzip into `tuning/climate/data/ascii/Koeppen-Geiger-ASCII.txt` (see `tuning/climate/README.md` for exact commands and a Wayback-mirror fallback).
